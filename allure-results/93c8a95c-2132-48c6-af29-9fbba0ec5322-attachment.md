...
- Location: tests/ui/download.spec.js:6:5
...
- generic [ref=e4]:
  ...
  - generic [ref=e26]:
    ...
    - generic [ref=e32]:
      ...
      - generic [ref=e37]: "|"
      - button "Download Forms" [active] [ref=e38] [cursor=pointer]:
    ...
    - generic [ref=e66]:
      ...
      - generic [ref=e77]:
        ...
        - generic [ref=e106] [cursor=pointer]:
          - img "Image" [ref=e110]
          - link "Click here to download Govt.Savings Scheme-Form 15 Letter of Indeminity" [ref=e114]:
...
```ts
  ...
  5  | 
  6  | test('Download file test', async ({ page ,context}) => {
  7  |     const downloadPath = path.join(__dirname, 'downloads');
  8  |     if (!fs.existsSync(downloadPath)) {
  9  |         fs.mkdirSync(downloadPath);
  10 |     }
  11 | 
  12 |     await page.goto('https://crh.sbi.bank.in/#FORMS');
  13 |     await page.locator("button[name='DownloadFormsButton']").click();
  14 |     await page.waitForTimeout(2000); // Wait for download to start
  15 |     const [download] = await Promise.all([
> 16 |         context.waitForEvent('download'),
     |                 ^ Error: browserContext.waitForEvent: Test ended.
  17 |         page.locator(text='Download').first().click()
  18 |     ]);
  19 | 
  20 |     const filePath = await download.path();
  21 |     const fileName = download.suggestedFilename();
  22 |     const savePath = path.join(downloadPath, fileName);
  23 |     
  24 |     await download.saveAs(savePath);
  25 |     
  26 |     expect(fs.existsSync(savePath)).toBeTruthy();
  27 | })
```