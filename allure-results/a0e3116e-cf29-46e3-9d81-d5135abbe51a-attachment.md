...
```
ReferenceError: typeofresponseBody is not defined
```
...
```ts
  ...
  42 |     expect(responseBody.booking.firstname).toBe(createBookingPayload.firstname)
> 43 |     expect(typeofresponseBody.booking.lastname).toBe('string')
     |            ^ ReferenceError: typeofresponseBody is not defined
  44 | })
```